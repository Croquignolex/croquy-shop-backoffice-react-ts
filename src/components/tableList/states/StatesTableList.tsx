...
import {StatesHookType} from "../../../pages/states/statesData";

const StatesTableList: FC<StatesTableListProps> = ({showCountry, addStatePath, fetchStates, statesBaseUrl}): ReactElement => {
    const {
    ...
    }: StatesHookType = useStatesTableListHook({fetchStates, statesBaseUrl});

    return (
        <Stack>
            <DisplayAlert data={statesAlertData} />
            <ListHeader
                label={"Ajouter une ville"}
                addItemPath={addStatePath}
                handleSearch={(needle: string) => fetchPaginatedNeedleStates(needle)}
            />
